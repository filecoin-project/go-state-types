...
import (
	...
	miner12 "github.com/filecoin-project/go-state-types/builtin/v13/miner"
	adt12 "github.com/filecoin-project/go-state-types/builtin/v13/util/adt"
...
type minerMigrator struct {
	...
	emptyDeadlinev13  cid.Cid
	emptyDeadlinesv13 cid.Cid
	OutCodeCID        cid.Cid
}
...
func newMinerMigrator(ctx context.Context, store cbor.IpldStore, outCode cid.Cid) (*minerMigrator, error) {
	...
	edv13, err := miner12.ConstructDeadline(ctxStore)
	if err != nil {
		return nil, xerrors.Errorf("failed to construct empty deadline v13: %w", err)
	}

	edv13cid, err := store.Put(ctx, edv13)
	if err != nil {
		return nil, xerrors.Errorf("failed to put empty deadline v13: %w", err)
	}

	edsv13 := miner12.ConstructDeadlines(edv13cid)
	edsv13cid, err := store.Put(ctx, edsv13)
	if err != nil {
		return nil, xerrors.Errorf("failed to construct empty deadlines v13: %w", err)
	...
	return &minerMigrator{
		...
		emptyDeadlinev13:  edv13cid,
		emptyDeadlinesv13: edsv13cid,
		OutCodeCID:        outCode,
	}, nil
}
...
func (m minerMigrator) migrateDeadlines(ctx context.Context, store adt11.Store, cache migration.MigrationCache, minerAddr address.Address, deadlines cid.Cid) (cid.Cid, error) {
	if deadlines == m.emptyDeadlinesV11 {
		return m.emptyDeadlinesv13, nil
	}
	...
	for i, c := range inDeadlines.Due {
		if c == m.emptyDeadlineV11 {
			outDeadlines.Due[i] = m.emptyDeadlinev13
		} else {
